...
const TweetMedia: React.FC<ITweetMediaProps> = ({ images, videos, tweetId, isVisible = true }) => {
  ...
  const player0 = useVideoPlayer(videoUrls[0]?.url || '', (player) => {
    if (videoUrls[0]) {
      player.loop = true;
  ...
  const player1 = useVideoPlayer(videoUrls[1]?.url || '', (player) => {
    if (videoUrls[1]) {
      player.loop = true;
      player.muted = isMuted;
    }
  ...
  const player2 = useVideoPlayer(videoUrls[2]?.url || '', (player) => {
    if (videoUrls[2]) {
      player.loop = true;
      player.muted = isMuted;
    }
  ...
  const player3 = useVideoPlayer(videoUrls[3]?.url || '', (player) => {
    if (videoUrls[3]) {
      player.loop = true;
      player.muted = isMuted;
    }
  ...
  useEffect(() => {
    ...
    if (shouldPlayVideo) {
      ...
      Object.values(videoPlayers).forEach((player) => player?.pause());
    }
